...
import org.flexiblepower.driver.zenobox.Zenobox.Config;
...
public class Zenobox
                    extends
                    AbstractResourceDriver</* ZenoboxState */PowerStateImpl, ZenoboxControlParameters>
                                                                                                      implements
                                                                                                      ...
                                                                                                      MqttCallback
{
    ...
    interface Config {
        @Meta.AD(deflt = "ZenoBoxDriver", description = "Resource identifier")
        String resourceId();
    ...
    class State implements ZenoboxState, PowerState {
        ...
        public Measurable<Power> getCurrentUsage() {
            logger.info("State Usage Examined...");
            final Measurable<Power> demand = Measure.valueOf(1.22, SI.WATT);
            return demand;
        }
    }
    ...
    public void activate(BundleContext context, Map<String, Object> properties) throws Exception {
        try {
            // Load configuration & set initial mode
            config = Configurable.createConfigurable(Config.class, properties);
            mode = ZenoboxMode.OFFLINE;

            // Configure MQTT - chose unique device id
            ...
            scheduledFuture = this.context.scheduleAtFixedRate(this,
                                                               ...
                                                               Measure.valueOf(config.updateInterval(),
                                                                               SI.SECOND));
    ...
    public synchronized void run() {
        ...
        // publishState(new State(mode, currentTemperature));
        publishState(getCurrentState_b());

        // Generate MQTT messages - to poll ZenoBox
        try {
    ...
    /*
     * @Override protected void handleControlParameters(ZenoboxControlParameters controlParameters) { // TODO
    ...
    protected PowerStateImpl getCurrentState_b() {
        final Measurable<Power> demand = Measure.valueOf(1.22, SI.WATT);
